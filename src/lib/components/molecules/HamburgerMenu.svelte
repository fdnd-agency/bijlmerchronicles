<script>
    ...
    $effect(() => {
        ...
        function handleClick() {
            const hamburgerMenuIsOpen =
                hamburgerButton.classList.toggle('hamburger-is-open');
        ...
        function cleanUp() {
            ...
            hamburgerButton.removeEventListener(
                'animationend',
                handleAnimationEndOnButton,
            );
...
<nav
    ...
    id="hamburgermenu"
    aria-label="Hamburger menu navigatie"
    popover="auto"
>
    <ul>
...
<style>
    ...
    @media (prefers-reduced-motion: no-preference) {
        :global(.slide-in-text) {
            animation: slide-in-text var(--hamburger-closing-opening-duration)
    ...
    @media (prefers-reduced-motion: no-preference) {
        :global(.open) {
            animation: slide-in var(--hamburger-closing-opening-duration)
                ease-in forwards;
        }

        :global(.closing) {
            animation: slide-out var(--hamburger-closing-opening-duration)
                ease-in forwards;
            background-color: var(--background-hamburger-pop-up);
        }
    }
